...
//-------- Manage Session ----------------//
public class C_ManageSession extends JPanel
{
    ...
    // New: Map to store student ID and their registered presentation type (Oral/Poster)
    private HashMap<String, String> studentTypeMap = new HashMap<>();
    ...
    // Modified: Now filters out students who are already booked in seminars.txt
    private String[] getStudents() 
    {
        java.util.ArrayList<String> list = new java.util.ArrayList<>();
        list.add("-- Select student --"); 
        
        // New: Clear the map to rebuild it
        studentTypeMap.clear();

        // New: Get list of students already assigned to a session
        HashSet<String> bookedStudents = new HashSet<>();
        try {
            ...
            if (f.exists()) {
                ...
                while (sc.hasNextLine()) {
                    ...
                    String[] parts = line.split(",");
                    if (parts.length >= 5) {
                        bookedStudents.add(parts[4].trim()); // Student ID is index 4
                    }
                }
                sc.close();
            }
        ...
        try {
            ...
            while (sc.hasNextLine()) { //keep reading, until the end of the line
                ...
                if (parts.length >= 6) {
                    ...
                    String type = parts[2].trim(); // Get Oral/Poster type
                    
                    // New: Store type mapping for auto-detection
                    studentTypeMap.put(stdID, type);

                    // New: Only add to dropdown if NOT already booked
                    if (!bookedStudents.contains(stdID)) {
                        list.add(stdID); 
                    }
                }
            }
    ...
    // New Function: Refreshes the student dropdown content dynamically
    ...
    public C_ManageSession()
    {
        ...
        gbc.gridx = 1;
        String[] sessionType = {"Oral", "Poster"}; // Fixed to match registration types
        cb = new JComboBox<>(sessionType);
        cb.setEnabled(false); // New: Disable manual selection, system will choose based on student
        add(cb, gbc);
        ...
        // New: Listener to auto-detect Presentation Type when student is selected
        studentBox.addActionListener(e -> {
            String selected = (String) studentBox.getSelectedItem();
            if (selected != null && studentTypeMap.containsKey(selected)) {
                String type = studentTypeMap.get(selected);
                cb.setSelectedItem(type); // Auto-set Oral or Poster
            }
        });
    ...
    private void populateFieldsFromSelection() {
        ...
        if (index > 0 && (index - 1) < allSessionLines.size()) { // index-1, to compare actual index in the dropdown with index in seminars.txt
            ...
            if (parts.length >= 5) {
                // New Logic: If editing, ensure the assigned student is visible in the dropdown
                // (because getStudents() filters them out by default)
                String currentStudent = parts[4].trim();
                boolean exists = false;
                for(int i=0; i<studentBox.getItemCount(); i++) {
                    if(studentBox.getItemAt(i).toString().equals(currentStudent)) exists = true;
                }
                if(!exists) {
                    studentBox.addItem(currentStudent);
                }
                ...
                studentBox.setSelectedItem(currentStudent);
            }
        } else {
            ...
            refreshStudentList(); // New: Reset list to hide booked students
    ...
    private void saveNewSession() {
        ...
        if (venue.isEmpty() || date.isEmpty()) {
            JOptionPane.showMessageDialog(C_ManageSession.this, "Please fill in all fields.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(C_ManageSession.this, "Session Saved Successfully!");
            venueTextField.setText("");
            
            // Refresh list so the new item appears in dropdown
            refreshSessionList();
            
            // New: Refresh student list to remove the student just booked
            refreshStudentList();

        } catch (java.io.IOException ex) {
            JOptionPane.showMessageDialog(C_ManageSession.this, "Error saving file: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void updateSelectedSession() {
        ...
        // New: Refresh list to ensure data consistency
        refreshStudentList();
    }
    ...
    private void deleteSelectedSession() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            // New: Student becomes available again, so refresh list
            refreshStudentList();
        }
    }
